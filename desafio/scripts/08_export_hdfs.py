...
for entidade in entidades:
    data = spark.sql("select * from {}.tbl_{}".format(DB_STG, entidade))
    data = process_dataframe_rdd(data, _types[entidade])
    logging.info("Exportando {}/{}/{}.csv".format(HDFS_GOLD_DIR, entidade, entidade))
    save_df(_df=data, _dir=HDFS_GOLD_DIR, _name=entidade)
    logging.info("Exportação de tabelas realizada com sucesso")


# how to run:
# docker exec jupyter-spark /opt/spark-2.4.1-bin-without-hadoop/bin/spark-submit desafio/scripts/02_export_hdfs.py